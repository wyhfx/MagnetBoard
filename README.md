...
### 刮削工具配置
1. **下载器配置**：在系统设置中配置qBittorrent等下载器
2. **推送设置**：设置推送规则，自动添加标签
3. **刮削工具配置**：配置MDC等刮削工具监听下载器目录
4. **自动刮削**：下载完成后刮削工具自动刮削元数据

### 媒体服务器集成
1. **目录结构**：刮削工具生成标准目录结构
2. **媒体服务器配置**：在Emby、Plex等媒体服务器中添加影视库目录
3. **自动识别**：媒体服务器自动识别并建立影视库
4. **元数据显示**：显示演员、封面、简介等信息
